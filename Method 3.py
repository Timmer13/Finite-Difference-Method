...
while t < tend:
    ...
    uRHS = -u*ddx(u,dx)-ddx(u,dx)*u/2 - ddy(v,dy)*u/2-v*ddy(u,dy)
    vRHS = -u*ddx(v,dx)-ddx(u,dx)*v/2 - ddy(v,dy)*v/2-v*ddy(v,dy)

    
    uh = u + dt*ν*laplacian(u,dx,dy) + dt*uRHS
    vh = v + dt*ν*laplacian(v,dx,dy) + dt*vRHS
